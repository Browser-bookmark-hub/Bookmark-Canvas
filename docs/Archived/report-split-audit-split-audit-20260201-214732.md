...
本轮变更：无代码改动（仅补充扫描/报告）。
注意：knip 误报 bookmark_canvas_module.js 未使用（script src 带 ?v=3.7）。

---
## SPLIT-AUDIT-ADDENDUM-20260202-0050
时间：2026-02-02 00:50

本轮变更（画布）：
- 绑定“标识管理”菜单交互（打开/关闭、清除、开关、自动清除设置）。
- initCanvasView 中新增调用 setupMarkerDropdownControls，确保永久栏目创建后即可使用菜单。

涉及文件：
- history_html/bookmark_canvas_module.js

---
## SPLIT-AUDIT-ADDENDUM-20260202-0100
时间：2026-02-02 01:00

本轮变更（画布）：
- 默认开启“自动定时清除”标识（autoClearEnabled=true）。
- 标识菜单支持“预设选择 + 自定义输入（分钟/小时/天）”。
- 交互逻辑同步更新：预设选择会回填自定义输入，自定义输入会切换到“自定义”。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0110
时间：2026-02-02 01:10

本轮变更（画布）：
- 修复标识菜单交互：点击外部关闭改为 click（避免原生下拉选项被误判为外部点击）。
- 菜单内控件阻止冒泡，确保可选、可输入。
- 数字输入去掉上下调节按钮。
- 标识开关项图标随状态切换（显示/隐藏）。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0125
时间：2026-02-02 01:25

本轮变更（画布）：
- 标识设置改为“面板式 UI”，从永久栏目标题区移到标题下方，避免拖拽捕获导致的不可选/不可输入问题。
- 菜单改为显式打开/关闭面板（支持关闭按钮与 ESC）。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0135
时间：2026-02-02 01:35

本轮变更（画布）：
- 修复永久栏目标题区域闭合，确保标识面板不再被拖拽标题区域包裹。

涉及文件：
- history_html/history.html

---
## SPLIT-AUDIT-ADDENDUM-20260202-0145
时间：2026-02-02 01:45

本轮变更（画布）：
- 标识设置回归“悬浮下拉”UI，并修复拖拽拦截导致的不可选/不可输入问题。
- 永久栏目拖拽逻辑：点击动作区/下拉菜单不触发拖拽。
- Popover 统一状态维护支持 marker dropdown。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0200
时间：2026-02-02 02:00

本轮变更（画布）：
- 标识 UI 改回悬浮下拉样式，并保留自定义时间输入/单位。
- 修复拖拽标题区域拦截：点击动作区/下拉菜单不触发拖拽，允许选择与输入。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0215
时间：2026-02-02 02:15

本轮变更（画布）：
- 永久栏目结构恢复为参考组原始布局（移除标识下拉 UI，修正标题/提示容器嵌套）。
- 恢复拖拽逻辑到参考组版本（不再包含 marker 下拉相关判断）。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0225
时间：2026-02-02 02:25

本轮变更（画布）：
- 恢复“清除变化标识”悬浮下拉菜单（动作区内）。
- 预设与自定义输入合并为同一行（不再出现“自定义”独立行）。
- 保留自动清除开关与单位选择，交互与拖拽冲突已规避。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0235
时间：2026-02-02 02:35

本轮变更（画布）：
- “间隔”候选 + 自定义输入合并为单一输入框（支持下拉候选 + 直接输入）。
- 输入解析支持分钟/小时/天（中英），自动格式化回填。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0245
时间：2026-02-02 02:45

本轮变更（画布）：
- “间隔”控件改为单一输入框 + 自定义下拉列表（可输入也可选）。
- 下拉列表高度与条目高度已收敛，避免过高。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0255
时间：2026-02-02 02:55

本轮变更（画布）：
- 候选下拉滚动不再被画布滚轮拦截；支持触控板滚动。
- 候选列表高度与条目高度收敛。
...
---
## SPLIT-AUDIT-ADDENDUM-20260202-0310
时间：2026-02-02 03:10

本轮变更（画布）：
- 候选列表改为「4档小时 + 3档天数」并支持中英文显示。
- 输入解析增强：支持小时/天/分钟的中英文与复数形式。
