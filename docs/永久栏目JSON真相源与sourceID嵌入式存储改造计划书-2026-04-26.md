# 永久栏目 JSON 真相源与 sourceID 嵌入式存储改造计划书（精简版，2026-04-26）
...
本次改造的核心目的不是单纯换字段，而是让插件内部永久栏目 JSON 和“推送到云端的永久栏目 JSON”保持同一套结构。这样同步时可以直接用云端 JSON 覆盖插件内部对应 JSON，再执行后续投影/刷新流程，不再靠 Chrome API 树和 sidecar map 临时拼出一份永久栏目数据。
...
4. Chrome Bookmarks API 只做执行层/投影层，不再做永久栏目长期真相源。
...
9. 原有函数名、桥接 API、调用入口不要随意重命名；优先保留名称并替换内部实现，必须改名时要保留兼容 wrapper 并一次性更新所有调用点。
...
1. 当前 sidecar key：`bcs:perm:source-id-map`
2. 当前 map 语义：`Chrome ID -> sourceID`
3. 当前导出/同步时，会从 Chrome API 树和 map 拼出永久 JSON。
...
1. `tree` 内所有 folder/bookmark 节点必须有 `sourceID`。
...
5. 从云端 pull 覆盖后，先得到没有本地 `id` 的永久树；投影到 Chrome Bookmarks API 后，必须把 Chrome 分配的新 `id` 回写进本地 `bcs:perm:main.tree` 节点。
...
说明：这里的本地 Chrome ID 优先使用现有 Chrome Bookmarks API 字段名 `id`，不要新造 `chromeId` 字段，除非代码里已经有明确兼容层。语义必须是“嵌入本地永久树节点”，不要再新建独立 Chrome ID 存储表或映射表。
...
3. 导出/上传时剥离本地 `id`。
4. 拉取云端 JSON 覆盖后，投影 Chrome API，并把新 `id` 回写到本地节点。
...
3. 用 `bcs:perm:main.tree` 投影到 Chrome Bookmarks API。
4. 投影成功后把 Chrome API 分配的新 `id` 回写到本地 `bcs:perm:main.tree` 对应节点。
...
```text
云端 JSON -> 校验 -> 直接覆盖插件内部永久 JSON -> 投影 Chrome Bookmarks API -> 回写 Chrome API id 到本地节点 -> 刷新主栏/副本视图
```

这里“直接覆盖”的对象是永久树和同步字段。云端 JSON 没有 Chrome API 本地 `id`，覆盖后由 Chrome API 投影结果重新写回本地节点；副本显示状态不跟随主树直接覆盖。
...
这些操作必须先改 `bcs:perm:main.tree`，再投影/同步到 Chrome API：
...
同步 pull 或批量投影期间，需要 suppress 自己触发的事件回声。
...
- [ ] 写迁移函数：Chrome API tree + 旧 map -> `bcs:perm:main.tree`。
- [ ] 迁移时一次性补齐缺失 `sourceID`。
...
- [ ] pull 后把 `bcs:perm:main.tree` 投影到 Chrome API。
- [ ] 投影成功后把新的 Chrome API `id` 回写到本地永久树节点。
...
### P5：移除 sidecar truth source

- [ ] 停用 `bcs:perm:source-id-map` 的真相源角色。
- [ ] 停用 `bcs:perm:source-id-export-keys`。
- [ ] 移除 `persistPermanentSourceIDMapAfterApply` 主链路。
- [ ] 守门脚本阻止重新引入 sidecar truth source。
- [ ] 原有函数名和 bridge API 尽量保留；确实不再适用的函数保留 wrapper 或迁移说明，避免调用链断裂。
