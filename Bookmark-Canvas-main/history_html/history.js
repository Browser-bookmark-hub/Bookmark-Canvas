    ...
    try {
        ...
        if (startIndex === 0) {
            ...
            if (treeRoot && treeRoot._existingLoadedCounts) {
                const memory = treeRoot._existingLoadedCounts[String(parentId)];
                if (memory) {
    ...
    try {
        ...
        for (let i = currentCount; i < childrenForRender.length; i++) {
            ...
            try {
                ...
                const tempDiv = document.createElement('div');
                tempDiv.innerHTML = html;
                const node = tempDiv.querySelector(':scope > .tree-node') || tempDiv.firstElementChild;
                ...
                try {
                    ...
                    if (expanded) {
                        ...
                        if (CANVAS_PERMANENT_TREE_LAZY_ENABLED && (currentView === 'canvas' || isReadOnlyChangesPreview)) {
                            ...
                            if (treeItem) {
                                try { treeItem.scrollIntoView({ block: 'start', behavior: 'smooth' }); } catch (_) {}
                            }
                        }
                    }
                } catch (_) { }
