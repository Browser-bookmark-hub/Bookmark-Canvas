(function setupCanvasSidebarDirectory(global) {
  ...
  function isCurrentViewport(item, currentCoords = null) {
    if (currentCoords) {
      const dx = Math.abs(item.x - currentCoords.x);
      const dy = Math.abs(item.y - currentCoords.y);
      const dz = Math.abs(item.zoom - currentCoords.zoom);
      return dx <= 1.0 && dy <= 1.0 && dz <= 0.001;
    }
    const state = global.CanvasModule && global.CanvasModule.CanvasState;
    if (!state) return false;
    const dx = Math.abs(item.x - state.panOffsetX);
    const dy = Math.abs(item.y - state.panOffsetY);
    const dz = Math.abs(item.zoom - state.zoom);
    return dx <= 1.0 && dy <= 1.0 && dz <= 0.001;
  }

  function updateActiveHistoryHighlight(currentCoords = null) {
    const panel = document.getElementById('canvasHistoryPanel');
    if (!panel || panel.style.display === 'none') return;
    
    // 1. Update auto-saved history list cards
    let historyList = [];
    try {
    ...
    if (Array.isArray(historyList)) {
      const cards = panel.querySelectorAll('.history-items-container .history-item-card');
      cards.forEach(card => {
        const index = parseInt(card.dataset.index, 10);
        const item = historyList[index];
        if (!item) return;
        
        const isActive = isCurrentViewport(item, currentCoords);
        if (isActive) {
    ...
    // 2. Update manual slot cards
    const slots = loadAnchorSlots();
    const slotCards = panel.querySelectorAll('.anchor-slots-container .anchor-slot-row');
    ...
    slotCards.forEach((card, idx) => {
      ...
      const isActive = isCurrentViewport(slot, currentCoords);
      if (isActive) {
        card.classList.add('active');
      } else {
        card.classList.remove('active');
  ...
  function getCanvasOtherSettingsSafe() {
    ...
    return { autoRecordAnchor: true, autoRecordAnchorInterval: 7 };
  }
  ...
  function renderHistoryPanel() {
    ...
    const autoRecordInterval = otherSettings.autoRecordAnchorInterval || 7;
    ...
    if (savedSlots.length === 0) {
      ...
      savedSlots.forEach((slot) => {
        ...
        const isActive = isCurrentViewport(slot);
        ...
        html += `
          <div class="anchor-slot-row ${activeClass}">
    ...
    if (currentHistoryList.length === 0) {
      ...
      visibleHistoryList.forEach((item, index) => {
        const isActive = isCurrentViewport(item);
        const activeClass = isActive ? 'active' : '';
        ...
        html += `
          <div class="anchor-slot-row history-item-card ${activeClass}" data-index="${index}">
            <div class="anchor-slot-header">
              <span class="anchor-slot-name" title="${displayName}">${displayName}</span>
              <div class="anchor-slot-actions">
  ...
  function init() {
    ...
    refreshTimer = global.setInterval(() => {
      ensureCanvasObserver();
      queueRefresh();
      try { updateActiveHistoryHighlight(); } catch (_) {}
    }, REFRESH_INTERVAL_MS);
    ...
    global.addEventListener('canvas-viewport-changed', (e) => {
      try { updateActiveHistoryHighlight(e && e.detail); } catch (_) {}
    });
  }
