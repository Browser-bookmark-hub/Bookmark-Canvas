    ...
    */
    ...
    compatText = isEn
        ? [
            ...
            '```text',
            '<export-root>/',
            '├── AGENTS.md                              (this AI editing guide)',
            '├── <canvas-entry>.canvas                  (canvas entry: nodes, edges, file mappings, text cards, groups, connectors)',
            '├── Permanent/                             (permanent section JSON folder; Chinese export: 永久栏目/)',
            '│   ├── A-PermanentBookmarks.json          (main permanent section, slot A; browser bookmark-tree snapshot)',
            '│   └── B-PermanentBookmarks.json          (optional permanent copy anchor, slot B; description/view only)',
            ...
            '    │   ├── A-1 <title>.json               (top-level regular chain; sequenceNumber 1 => A-1)',
            '    │   ├── A-1-1 <title>.json             (derived chain; may inherit most content from A-1)',
            '    │   └── B-1 <title>.json               (top-level regular chain; sequenceNumber 2 => B-1)',
            '    └── Special temporary/                 (special bookmark sandboxes; Chinese export: 特殊临时栏目/)',
            '        ├── AI <title>.json                (default target for AI-generated bookmark sets)',
            '        └── Add/Search/Import <title>.json (other special temporary sections)',
            '```',
            ...
            '- The exported `tree.id` and `tree.parentId` values are `syncId_*`, not local Chrome numeric IDs. When adding permanent nodes, mint unique IDs shaped like `syncId_YYYYMMDD_hash_<token7>`, where `token7` is lowercase letters/digits only. See [R1](#ref-r1).',
            ...
            '- Top-level regular labels are generated from `sequenceNumber` through `toAlphaLabel()`: 1 => A-1, 2 => B-1, 3 => C-1, 27 => AA-1. See [R1](#ref-r1).',
            ...
            '- Preserve existing plugin-style IDs such as `permanent-section`, `temp-section-A-1`, `card-group-*`, `md-node-*`, and `edge-*`; do not convert them to generic 16-hex IDs.',
            '',
        ...
        : [
            ...
            '```text',
            '<导出包根目录>/',
            '├── AGENTS.md                                 （当前 AI 编辑指南）',
            '├── <画布入口>.canvas                         （画布入口：nodes、edges、文件映射、空白栏目、卡片组、连接线）',
            '├── 永久栏目/                                 （永久栏目 JSON 文件夹；对应浏览器书签树）',
            '│   ├── A书签树（永久栏目）.json              （主永久栏目，slot A；永久书签树快照规范来源）',
            '│   └── B书签树（永久栏目）.json              （可选永久栏目副本锚点，slot B；只保留副本说明/视图）',
            '└── 临时栏目/                                 （临时栏目 JSON 文件夹；书签沙盒）',
            '    ├── 常规链式/                             （普通链式书签沙盒）',
            '    │   ├── A-1 <标题>.json                   （sequenceNumber 1 的顶层普通链式）',
            '    │   ├── A-1-1 <标题>.json                 （派生链式；可承接 A-1 的大部分内容）',
            '    │   └── B-1 <标题>.json                   （sequenceNumber 2 的顶层普通链式）',
            '    └── 特殊临时栏目/                         （特殊临时书签沙盒）',
            '        ├── AI <标题>.json                    （AI 生成书签树的默认落点）',
            '        └── 添加/搜索/导入 <标题>.json        （其他特殊临时栏目）',
            '```',
            ...
            '- 导出包里的 `tree.id` / `tree.parentId` 是 `syncId_*`，不是本机 Chrome 数字 ID。新增永久节点时生成唯一 `syncId_YYYYMMDD_hash_<token7>`，其中 `token7` 只能是小写字母/数字。见 [R1](#ref-r1)。',
            ...
            '- 顶层普通链式标号由 `sequenceNumber` 经过 `toAlphaLabel()` 得到：1 => A-1，2 => B-1，3 => C-1，27 => AA-1。见 [R1](#ref-r1)。',
            ...
            '- 保留插件现有 ID 风格，如 `permanent-section`、`temp-section-A-1`、`card-group-*`、`md-node-*`、`edge-*`；不要统一改成 16 位十六进制 ID。',
            '',
            '### A6. tag 系统协议',
    ...
    aiGuideText = isEn
        ? [
            ...
            '',
            'Route principle: edit the minimum file set only. Prefer adding AI-generated bookmark data as a special temporary section unless the user explicitly asks to edit the permanent browser tree.',
            '',
            '### S1. File Routing (what to touch)',
            `- Permanent main content: edit \`${permanentMdRel}\` only for intentional browser-bookmark-tree changes.`,
            ...
            '',
            '### S2. AI-generated Bookmarks',
            '- Default target: create or update a special temporary section with `label: "AI"` and `tempKind: "special"`. See [R5](#ref-r5).',
            '- If the package only has a permanent section and AI needs to add new bookmark suggestions, create `Temporary/Special temporary/AI <title>.json` and add a matching file node to `.canvas`. See [R7](#ref-r7).',
            '- Use `source: "ai-generated"` and a meaningful `descriptionMd` explaining what the generated bookmark tree contains. See [R5](#ref-r5).',
            '- Do not merge AI-generated suggestions into permanent bookmarks unless the user explicitly asks for that.',
            '',
            '### S3. ID and Numbering Rules',
            '- Permanent new nodes: `syncId_YYYYMMDD_hash_<token7>`; `token7` is lowercase letters/digits only, and `parentId` must point to the parent syncId. See [R1](#ref-r1).',
            '- Temporary new items: `tempId_YYYYMMDD_hash_<token7>`; `token7` is lowercase letters/digits only, and every nested item keeps the section ID in `sectionId`. See [R1](#ref-r1).',
            '- Regular temporary top-level labels: `{Alpha}-1` from `sequenceNumber` by `toAlphaLabel()`; examples: 1 A-1, 2 B-1, 3 C-1, 27 AA-1. See [R1](#ref-r1).',
            ...
            '- Special temporary IDs use the label and duplicate suffixes, for example `temp-section-AI`, `temp-section-AI-2`. See [R1](#ref-r1).',
            '',
            '### S4. Color Rules',
            ...
            '',
            '### S5. Minimal Pre-import Checklist',
        ...
        : [
            ...
            '',
            '路由原则：只改最小必要文件集合。AI 新生成的书签、书签树默认放入特殊临时栏目；只有用户明确要求修改浏览器书签树时，才改永久栏目。',
            '',
            '### S1. 文件路由（按需求定位）',
            `- 永久栏目主内容：只有明确要改浏览器书签树时才编辑 \`${permanentMdRel}\`。`,
            ...
            '',
            '### S2. AI 生成书签的默认落点',
            '- 默认新建或更新一个特殊临时栏目，使用 `label: "AI"` 与 `tempKind: "special"`。见 [R5](#ref-r5)。',
            '- 如果包里只有永久栏目，而 AI 需要新增书签建议，应创建 `临时栏目/特殊临时栏目/AI <标题>.json`，并在 `.canvas` 增加匹配的 file 节点。见 [R7](#ref-r7)。',
            '- 使用 `source: "ai-generated"`，并用 `descriptionMd` 说明这棵生成书签树的内容。见 [R5](#ref-r5)。',
            '- 除非用户明确要求，不要把 AI 生成建议直接合并进永久栏目。',
            '',
            '### S3. ID 与编号规则',
            '- 永久栏目新增节点：`syncId_YYYYMMDD_hash_<token7>`；`token7` 只能是小写字母/数字，`parentId` 必须指向父节点 syncId。见 [R1](#ref-r1)。',
            '- 临时栏目新增 item：`tempId_YYYYMMDD_hash_<token7>`；`token7` 只能是小写字母/数字，嵌套 item 的 `sectionId` 都必须等于所在栏目 ID。见 [R1](#ref-r1)。',
            '- 普通链式顶层标号：`{字母}-1`，字母由 `sequenceNumber` 经 `toAlphaLabel()` 得到；示例：1 A-1，2 B-1，3 C-1，27 AA-1。见 [R1](#ref-r1)。',
            '- 派生普通链式可以继续扩展，如 `A-1-1`；编辑时保留父子链式意图。见 [R4](#ref-r4)。',
            '- 特殊临时栏目 ID 使用标签和重复后缀，如 `temp-section-AI`、`temp-section-AI-2`。见 [R1](#ref-r1)。',
            '',
            '### S4. 颜色规则',
    ...
    const referenceExamplesText = isEn
        ? [
            ...
            '- Regular top-level labels: `{Alpha}-1`, where `Alpha = toAlphaLabel(sequenceNumber)`. Examples: sequenceNumber 1 => A-1, 2 => B-1, 3 => C-1, 27 => AA-1.',
            ...
            'Use this when AI adds suggested bookmarks or a generated bookmark tree.',
            '```json',
            '{',
            '  "format": "bookmark-canvas-section",',
            '  "schemaVersion": 2,',
            ...
            '<a id="ref-r6"></a>',
            '### R6. .canvas Example',
            'Paths in file nodes are vault-relative. Match the prefix style already used in the current `.canvas` file.',
            '```json',
            '{',
            '  "nodes": [',
            '    { "id": "permanent-section", "type": "file", "file": "<vault-relative Permanent/A-PermanentBookmarks.json>", "x": 0, "y": 0, "width": 600, "height": 600, "color": "4" },',
            '    { "id": "permanent-section-copy-permanent-copy-1780113045642-gtjhd", "type": "file", "file": "<vault-relative Permanent/B-PermanentBookmarks.json>", "x": 720, "y": 0, "width": 600, "height": 600, "color": "4" },',
            '    { "id": "card-group-ai", "type": "group", "x": -40, "y": 700, "width": 1320, "height": 620, "label": "AI workspace", "color": "5" },',
            '    { "id": "temp-section-AI", "type": "file", "file": "<vault-relative Temporary/Special temporary/AI title.json>", "x": 0, "y": 760, "width": 525, "height": 380, "color": "#e9973f" },',
            ...
            '  ],',
            '  "edges": [',
            '    { "id": "edge-ai-1", "fromNode": "md-node-ai-prompt", "fromSide": "right", "toNode": "temp-section-AI", "toSide": "left", "toEnd": "arrow", "color": "#999999", "label": "generated set" }',
            '  ]',
            '}',
            '```',
            '',
            '<a id="ref-r7"></a>',
            '### R7. If the Package Has Only One Permanent Section',
            'Do not edit the permanent tree just to add AI suggestions. Add an AI special temporary JSON file, add a file node in `.canvas`, and keep the permanent section unchanged unless the user explicitly asks for browser-bookmark changes.',
            '',
            '<a id="ref-r8"></a>',
        ...
        : [
            ...
            '- 普通链式顶层标号：`{字母}-1`，字母由 `sequenceNumber` 经 `toAlphaLabel()` 得到。示例：sequenceNumber 1 => A-1，2 => B-1，3 => C-1，27 => AA-1。',
            ...
            '<a id="ref-r6"></a>',
            '### R6. .canvas 示例',
            'file 节点路径是 vault 相对路径。新增路径要匹配当前 `.canvas` 已经使用的前缀风格。',
            '```json',
            '{',
            '  "nodes": [',
            '    { "id": "permanent-section", "type": "file", "file": "<vault相对 永久栏目/A书签树（永久栏目）.json>", "x": 0, "y": 0, "width": 600, "height": 600, "color": "4" },',
            '    { "id": "permanent-section-copy-permanent-copy-1780113045642-gtjhd", "type": "file", "file": "<vault相对 永久栏目/B书签树（永久栏目）.json>", "x": 720, "y": 0, "width": 600, "height": 600, "color": "4" },',
            '    { "id": "card-group-ai", "type": "group", "x": -40, "y": 700, "width": 1320, "height": 620, "label": "AI workspace", "color": "5" },',
            '    { "id": "temp-section-AI", "type": "file", "file": "<vault相对 临时栏目/特殊临时栏目/AI title.json>", "x": 0, "y": 760, "width": 525, "height": 380, "color": "#e9973f" },',
            '    { "id": "md-node-ai-prompt", "type": "text", "text": "Prompt or notes for this generated bookmark set.", "x": 600, "y": 760, "width": 420, "height": 260, "color": "#888888" }',
            '  ],',
            '  "edges": [',
            '    { "id": "edge-ai-1", "fromNode": "md-node-ai-prompt", "fromSide": "right", "toNode": "temp-section-AI", "toSide": "left", "toEnd": "arrow", "color": "#999999", "label": "generated set" }',
            ...
            '<a id="ref-r7"></a>',
            '### R7. 如果导出包只有一个永久栏目',
            '不要为了添加 AI 建议而改永久树。应新增一个 AI 特殊临时栏目 JSON 文件，在 `.canvas` 中增加对应 file 节点，并保持永久栏目不变；除非用户明确要求修改浏览器永久书签。',
            '',
            '<a id="ref-r8"></a>',
            '### R8. tag 调色板与 tag 对象',
